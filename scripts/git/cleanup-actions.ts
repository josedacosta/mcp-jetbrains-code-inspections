...
async function getWorkflows(): Promise<Workflow[]> {
    try {
        ...
        const result = executeCommand('gh api repos/josedacosta/mcp-jetbrains-code-inspections/actions/workflows --paginate');
...
async function getWorkflowRuns(workflowId?: string): Promise<WorkflowRun[]> {
    try {
        ...
        if (workflowId && !/^\d+$/.test(workflowId)) {
            runs = runs.filter((run: WorkflowRun) => run.name.toLowerCase().includes(workflowId.toLowerCase()));
        }
...
async function deleteWorkflowRun(runId: number): Promise<boolean> {
    ...
    try {
        executeCommand(`gh api -X DELETE repos/josedacosta/mcp-jetbrains-code-inspections/actions/runs/${runId}`);
        return true;
    } catch (error) {
...
async function main() {
    ...
    try {
        ...
        for (const [workflowName, workflowRuns] of Object.entries(runsByWorkflow)) {
            ...
            workflowRuns.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
            ...
            for (let i = 0; i < workflowRuns.length; i++) {
                ...
                if (isLast && options.keepLast) {
                    log(
                        `  ${emoji} #${run.run_number} - ${colors.yellow}KEPT${colors.reset} (most recent) - ${run.conclusion || run.status}`,
                        colors.gray,
                    );
                    skippedCount++;
                    continue;
                ...
                if (options.maxAge > 0 && daysOld < options.maxAge) {
                    log(
                        `  ${emoji} #${run.run_number} - ${colors.cyan}SKIPPED${colors.reset} (${daysOld} days old) - ${run.conclusion || run.status}`,
                        colors.gray,
                    );
                    tooRecentCount++;
                    continue;
                ...
                if (run.conclusion === 'success') {
                    ...
                    const statusColor = run.conclusion === 'failure' ? colors.red : run.conclusion === 'cancelled' ? colors.yellow : colors.gray;
