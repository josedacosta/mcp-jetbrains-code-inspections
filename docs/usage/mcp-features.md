...
const bestIDE = ides.detected.reduce((best, current) => (current.score > best.score ? current : best));
...
const validResources = ['inspection://profiles', 'inspection://config', 'inspection://ides'];
