...
async function generateQualityReport(paths) {
  ...
  for (const path of paths) {
    ...
    for (const diagnostic of diagnostics.diagnostics) {
      // By severity
      report.issuesBySeverity[diagnostic.severity] = 
        (report.issuesBySeverity[diagnostic.severity] || 0) + 1;
      
      // By file
      report.issuesByFile[diagnostic.file] = 
        (report.issuesByFile[diagnostic.file] || 0) + 1;
      
      // By inspection type
      report.topInspections[diagnostic.inspection] = 
        (report.topInspections[diagnostic.inspection] || 0) + 1;
    }
  }
...
async function generateReviewComment(files) {
  ...
  for (const file of files) {
    ...
    const criticalIssues = diagnostics.diagnostics.filter(d => 
